...
def unsigned_package_has_integrity_checks(workflow: str) -> bool:
    ...
    if job is None:
        return False
    return (
        "--bundle syndrid" in job
        and "Download unsigned Syndrid runtime binaries" in job
        and "Verify unsigned runtime inputs" in job
        and "Verify unsigned Syndrid package contents" in job
        and "Get-AuthenticodeSignature" not in job
    )
...
jobs:
  build-windows:
    steps:
      ...
      - name: Verify unsigned runtime inputs
"""
        self.assertTrue(unsigned_package_has_integrity_checks(safe))
        self.assertFalse(unsigned_package_has_integrity_checks(unsafe))
