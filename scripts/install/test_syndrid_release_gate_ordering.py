...
def load_contract_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "syndrid_release_contract_ordering", path
    )
...
class SyndridReleaseGateOrderingTests(unittest.TestCase):
    ...
    def seed_contract(
        self, root: Path, *, audit_after_release: bool, smoke_after_release: bool
    ) -> None:
        before_release = [
    ...
    def test_audit_after_release_is_blocker(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.seed_contract(
                root, audit_after_release=True, smoke_after_release=False
            )
    ...
    def test_smoke_after_release_is_blocker(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.seed_contract(
                root, audit_after_release=False, smoke_after_release=True
            )
    ...
    def test_audit_and_smoke_before_release_satisfy_ordering(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.seed_contract(
                root, audit_after_release=False, smoke_after_release=False
            )
