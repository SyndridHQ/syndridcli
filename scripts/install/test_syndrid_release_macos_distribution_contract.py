...
def load_contract():
    ...
    spec = importlib.util.spec_from_file_location("syndrid_release_contract_macos", path)
...
class SyndridReleaseMacOSDistributionContractTests(unittest.TestCase):
    def write(self, root: Path, path: str, content: str) -> None:
        full = root / path; full.parent.mkdir(parents=True, exist_ok=True); full.write_text(content, encoding="utf-8")
    ...
    def test_macos_production_markers_are_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self.seed(root, "build-macos:\n")
            result = contract.audit_release_contract(root)
            self.assertFalse(result["ok"])
            self.assertEqual([f["needle"] for f in result["blockers"]], ["build-macos:"])
